...
static int is_opencl(Renderer* r){ return r && (str_eq(r->suffix, "OPENCL") || str_eq(r->device, "QCOM")); }
static int is_metal(Renderer* r){ return r && str_eq(r->device, "METAL"); }
static int is_cuda(Renderer* r){ return r && (str_eq(r->device, "CUDA") || str_eq(r->device, "NV")); }
static int is_hip_or_amd(Renderer* r){ return r && (str_eq(r->device, "HIP") || str_eq(r->device, "AMD")); }
static int gep_threshold_for(Renderer* r){
  if (is_cuda(r)) return 8;
  if (is_opencl(r) || is_metal(r) || is_hip_or_amd(r)) return 4;
  return 0;
}
...
static const char* c_type_for_b(Renderer* r, const DType* dt){
  return is_opencl(r) ? c_type_for_opencl(dt) : c_type_for(dt);
}
static const char* c_vec_type_for_b(Renderer* r, const DType* dt){ (void)r; return c_vec_type_for(dt); }

static char* render_vec_literal(Renderer* r, const DType* dt, const char* elems) {
  const char* t = c_vec_type_for_b(r, dt);
  if (is_opencl(r)) {
...
static char* render_const(UOp* u){
  char buf[128];
  if (dtypes_is_float(&u->dtype)) {
    double v = u->arg.const_data.const_value;
    if (dtype_eq(&u->dtype, &dtypes.float32)) {
      if (isnan(v)) return strdup("__builtin_nanf(\"\")");
      if (isinf(v)) return strdup((v < 0) ? "-__builtin_inff()" : "__builtin_inff()");
      snprintf(buf, sizeof(buf), "%gf", (float)v);
    } else if (dtype_eq(&u->dtype, &dtypes.float64)) {
      if (isnan(v)) return strdup("__builtin_nan(\"\")");
      if (isinf(v)) return strdup((v < 0) ? "-__builtin_inf()" : "__builtin_inf()");
...
static char* clang_render(Renderer* self, UOp** uops, int uops_count) {
  ...
  for (int i=0;i<param_count;i++){ char seg[128];
    if (is_opencl(self) && param_is_image[i]) {
      ...
      snprintf(seg,sizeof(seg),"%s%s* data%d%s", pref, c_type_for_b(self, param_dtypes[i]), param_ids[i], (i<param_count-1)?", ":"");
    }
    strcat(fnhead, seg);
  }
  strcat(fnhead, ") {\n");
  ...
  for (int i=0;i<uops_count;i++){
    ...
    if (u->op==OPS_SQRT && u->src_count==1){
      const char* sn = ssa_get(&map, u->src[0]); char* sv = sn?strdup(sn):(u->src[0]->op==OPS_CONST?render_const(u->src[0]):strdup("0"));
      const char* nm = ssa_add(&map, u); char line[256]; INDENT();
    ...
    if (u->op==OPS_LOAD && u->src_count>=1){
      ...
      char* alt = NULL; if (u->src_count>=2){ const char* vn = ssa_get(&map, u->src[1]); alt = vn?strdup(vn):(u->src[1]->op==OPS_CONST?render_const(u->src[1]):strdup("0")); }
      ...
      if (is_opencl(self) && idxu && idxu->op==OPS_INDEX && idxu->src_count>=2){
        ...
        if (gid>=0 && dtypes_is_image(gdt)){
          const char* idxn = ssa_get(&map, idxu->src[1]); char* idxv = idxn?strdup(idxn):(idxu->src[1]->op==OPS_CONST?render_const(idxu->src[1]):strdup("0"));
    ...
    if (u->op==OPS_STORE && u->src_count>=2){
      ...
      char* cond = NULL; if (u->src_count>=3){ const char* cn = ssa_get(&map, u->src[2]); cond = cn?strdup(cn):(u->src[2]->op==OPS_CONST?render_const(u->src[2]):strdup("0")); }
      char line[768];
      ...
      bool did_img=false; if (is_opencl(self) && u->src[0]->op==OPS_INDEX){
        ...
        int gid=-1; const DType* gdt=&dtypes.float32; if (bt){ for(size_t k=0;k<tn;k++) if (bt[k]->op==OPS_DEFINE_GLOBAL){ gid=(bt[k]->arg.type==ARG_INT)? bt[k]->arg.int_data.i:0; gdt=&bt[k]->dtype; break; } free(bt);}        
        if (gid>=0 && dtypes_is_image(gdt)){
          const char* idxn = ssa_get(&map, idxu2->src[1]); char* idxv = idxn?strdup(idxn):(idxu2->src[1]->op==OPS_CONST?render_const(idxu2->src[1]):strdup("0"));
    ...
    if (u->op==OPS_VECTORIZE && u->src_count>0){
      ...
      for (size_t k=0;k<u->src_count;k++){
        const char* vn = ssa_get(&map, u->src[k]); char* vv = vn?strdup(vn):(u->src[k]->op==OPS_CONST?render_const(u->src[k]):strdup("0"));
        if (k>0) strcat(elems, ", ");
    ...
    if (u->op==OPS_WHERE && u->src_count==3){
      const char* cn = ssa_get(&map, u->src[0]); char* c = cn?strdup(cn):(u->src[0]->op==OPS_CONST?render_const(u->src[0]):strdup("0"));
      const char* tn = ssa_get(&map, u->src[1]); char* t = tn?strdup(tn):(u->src[1]->op==OPS_CONST?render_const(u->src[1]):strdup("0"));
      const char* fn = ssa_get(&map, u->src[2]); char* f = fn?strdup(fn):(u->src[2]->op==OPS_CONST?render_const(u->src[2]):strdup("0"));
      ...
      for (size_t p=0; p<L;){
        if (fmt[p]=='{' && p+1<L && fmt[p+1]=='}'){
          ...
          if (si < (int)u->src_count){
            if (u->src[si]->op == OPS_CONST) { free(rep); rep = render_const(u->src[si]); }
...
// Minimal stubs for additional C-style backends. These currently share the
// same renderer logic as Clang; language-specific differences (kernel attrs,
// vector literal style, image I/O) can be added incrementally as tests land.
...
Renderer* renderer_cstyle_metal(void) {
  ...
  r->has_local = true;
  r->has_shared = true;
  r->shared_max = 32*1024; // typical threadgroup memory
  return r;
}
